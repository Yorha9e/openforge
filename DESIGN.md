...
> 日期: 2026-06-05 | 比赛: Agent 辅助全栈挑战赛 | 题目: 题一「AI工程工具」 | 版本: v6 (代码优先审计 — 基于 208 Go + 87 前端 + 28 Node.js 文件直接扫描)
...
### 1.2b v6 代码优先审计状态 (2026-06-05)

> 本节基于代码直接扫描，不依赖历史文档。代码库: 208 Go + 87 前端 + 28 Node.js + 10 迁移 + 70 文档。
...
## 三、A层: Pipeline 引擎 `[v6 审计: 领域模型完成, 服务层完成]`

> **[v6 审计]** Pipeline 引擎代码组织完整。`internal/pipeline/domain/` 含 8 文件: `pipeline.go` (5.65KB, Pipeline 聚合根 + 状态机) / `stage.go` / `gate.go` (1.07KB, Gate 值对象) / `gate_hook.go` (1.08KB, Pre/Post 拦截器接口) / `file_lock.go` (2.28KB, 文件级并发锁) / `module_ownership.go` (1.62KB) / `complexity.go` (L1-L4 复杂度枚举)。`internal/pipeline/service/` 含 10 文件: `pipeline_service.go` / `gate_service.go` (3.08KB) / `deploy_service.go` (3.07KB) / `canary_service.go` / `file_lock_service.go` / `ownership_service.go` / `token_cost_service.go`。`internal/pipeline/adapter/` 含 PG 持久化实现: `pg_repository.go` (13.63KB) + `pg_repository_batch.go` (批量消息写入) + `pg_gate_request_repository.go` + `pg_file_lock.go`。测试覆盖: 每个 service 均有对应 `_test.go`。
...
### 4.1 Go 协调层 `[v6 审计: 核心完成]`

> **[v6 审计]** `internal/agent/domain/coordinator.go` (3.99KB) 已实现 `AgentCoordinator`，管理 `agents map[string]*AgentInstance`，支持 `Spawn/Delegate/Broadcast/Terminate/Chat/ChatStream/SearchTools/RunTool`。硬编码上限 `maxAgents=5`。`internal/agent/service/csp_channel.go` (1.39KB) 已实现带缓冲 Go channel + 内存 WAL 链表（上限 4096 条），WAL 为纯内存非持久化。`internal/agent/adapter/grpc_llm_client.go` (3.76KB) 桥接 Go → Node.js gRPC LLM 服务。
...
### 4.2 Query Engine (对话生命周期管理) `[Phase 1.5 — v5 spec 闭环, 评分 9.5/10]` `[v6 审计: 已实现]`
...
### 4.2b Prompt 构建系统 `[Phase 5d — v2 简化]` `[v6 审计: 核心完成, ProjectPrefs stub]`

> 架构: L1 (static.xml 不可覆盖) → L2 (项目融合) → L4 (对话摘要)。L3 退化入 L2（无独立数据源）。注入器精简为 KnowledgeQuerier（L2 附属）+ 独立 tools_stages.go。
>
> **[v6 审计]** 实际代码 `internal/agent/domain/prompt_builder.go` (13.85KB)。三层架构已实现: L1 (`config/prompts/static.xml`, 25行 XML, 启动加载内存缓存) → L2 (`L2Builder`: stage_templates.go 22.99KB 含 L1-L4 全级别模板 + KnowledgeQuerier 5min TTL 缓存) → L4 (`buildL4Summary` 保留最近 10 轮原文)。`CapabilityInjector` (5.1KB) 统一注入 Skill+Tool。**注意**: `ProjectPrefsLoader` 当前为 stub — `Get()` 和 `GetStageOverride()` 均返回空字符串，标注 Phase 6。组装顺序: `L2 + Capability XML + L4Summary + L1`。安全措施: `sanitizePrompt()` 移除注入模式。Token 估算: `len/4` 简单估算（非 tiktoken）。
...
### 4.3 Tool 接口标准化 `[Phase 1.5 — v4 新增]` `[v6 审计: 已实现, 11 工具]`

> 采纳自 Claude Code: 当前 OpenForge 工具仅在 Proto 层定义 RPC, Go 侧无 Tool 泛型接口, 每加一个 Tool 都是一次性代码。
>
> **[v6 审计]** 实际代码已完整实现 Tool 泛型体系。`internal/agent/port/tool.go` 定义 `Tool` 接口（Name/Description/Execute）。`internal/agent/port/tool_registry.go` (1.55KB) 定义 `ToolRegistryClientFull` 接口。`internal/agent/domain/tool_registry_builder.go` (15.69KB) 实现注册构建器。`internal/agent/domain/tool_executor.go` (4.82KB) 实现执行器。`internal/agent/domain/tool_proxy.go` + `tool_proxy_executor.go` 实现代理模式。`internal/tool/` 目录含 11 个工具实现: `bash_tool.go` / `read_tool.go` / `write_tool.go` / `edit_tool.go` / `replace_in_file_tool.go` / `delete_file_tool.go` / `grep_tool.go` / `glob_tool.go` / `search_file_tool.go` / `list_dir_tool.go` / `git_tool.go`。`tools_stages.go` (4.09KB) 按 stage 硬编码工具列表 + plan 模式只读过滤。
...
### 4.5 LLM Router + 模型注册表 `[Phase 1.5 — v5 新增]` `[v6 审计: Go 侧已实现, 双通道并存]`
...
>
> **[v6 审计]** Go 侧 LLM 子系统已实现: `internal/llm/registry.go` 模型注册表 + `router.go` 路由器 + `provider.go` Provider 接口 + 3 个 Provider 实现 (`anthropic_provider.go` 支持 tool use / `deepseek_provider.go` 继承 Anthropic 改 baseURL / `openai_provider.go` Chat Completions API)。`bootstrap.go` 在启动时注册模型到 Registry，QueryEngine 通过 `LLMRouter` 接口调用。**注意**: 当前存在双通道 — Go 侧 LLM Router 直连 API 与 Node.js gRPC LLMRouterService 并存。`grpc_llm_client.go` (3.76KB) 桥接 Go → Node.js gRPC，用于 WebSocket 聊天流式场景。Node.js 侧 chat/chatStream 仅路由 Anthropic（见 §4.6 审计）。
...
### 4.6 Node.js IO 层 `[v6 审计: 部分实现]`

> **[v6 审计]** 实际代码 `nodejs-io/src/server.ts` (7.71KB)。gRPC 服务运行在端口 50051，实现 LLMRouterService 5 个 RPC:
> - `chat` / `chatStream`: ✅ 通过 `providers` 映射表 + `resolveProvider()` 支持 Anthropic/DeepSeek/OpenAI 多供应商路由
> - `listModels`: 从环境变量动态解析模型列表 — 已完成
> - `switchModel`: 返回确认，下次对话生效 — 已完成
> - `getTokenUsage`: ✅ 对接 `TokenMeter.getSummary()` 返回真实聚合数据（按 provider 分组，BigInt 精度）
>
> **LLM Providers** (`nodejs-io/src/llm/providers/`):
> - `anthropic.ts` (2.74KB): 完整实现，支持 tool use，使用 `@anthropic-ai/sdk`
> - `deepseek.ts` (401B): 继承 AnthropicProvider，仅改 baseURL（利用 DeepSeek Anthropic 兼容端点）
> - `openai.ts` (3.02KB): 完整实现 Chat Completions API + SSE 流式，但 server.ts 未路由请求到此 provider；不支持 tool use
>
> **Proto 生成代码**: 6 服务 × 2 文件 = 12 文件 (coordinator/gate/learning/llm/terminal/tools)
...
Token/成本:
  ...
  GET    /api/projects/:id/token-budget          # Token 配额余量
...
```
端点: wss://<host>/ws/chat
鉴权: JWT 不放 URL query string。浏览器使用
      Sec-WebSocket-Protocol: openforge.auth, bearer.<token>
      在 Upgrade 前完成校验；缺失或无效令牌返回 401，不升级连接。
      非浏览器客户端可使用 Authorization: Bearer <token>。
...
**WebSocket 鉴权**: JWT 不放 URL query string。浏览器使用 `Sec-WebSocket-Protocol: openforge.auth, bearer.<token>` 在 Upgrade 前传递 BFF 签发的短期令牌；服务端在升级前校验，缺失或无效令牌返回 401。非浏览器客户端可使用 `Authorization: Bearer <token>`。
...
W3C Trace Context (前端侧):
  ├── 页面加载时生成 W3C traceparent → 随 REST/WS 业务消息传播
...
- 浏览器改用 `Sec-WebSocket-Protocol: openforge.auth, bearer.<token>`，服务端在 Upgrade 前校验，缺失或无效令牌返回 401
- 非浏览器客户端可使用 `Authorization: Bearer <token>`
...
## 六、安全性 `[v6 审计: 3 个高危缺口已关闭]`

> **Phase 2 前置**: 本章节 JWT/XSS/CSP 3 个高危项已在 Phase 2/3 安全闭环中修复。
>
> **[v6 审计] 修复记录** (2026-06-05):
> - **✅ OIDC JWT 签名验证**: `internal/auth/adapter/oidc_provider.go` 引入 `coreos/go-oidc/v3`，`Exchange()` 优先使用 JWKS 验证签名，仅在 provider discovery 失败时 fallback 到 `parseIDTokenUnsafe`（离线开发场景）。
> - **✅ Profile Ed25519 签名验证**: `internal/shared/profile/loader.go` 实现 `verifyEd25519Signature()`，使用 `crypto/ed25519` + hex 编码公钥（`OF_PROFILE_PUBKEY` 环境变量）。同时添加 `os.ExpandEnv` 环境变量展开支持。
...
#### 10.1.1 完整可插拔矩阵 `[v6 审计: 已验证 bootstrap.go 实际路由]`

> **[v6 审计]** 以下矩阵标注了 `bootstrap.go` 中 `new*()` 函数的实际路由逻辑。"✅ 已实现"表示该 profile 下有真实适配器代码且 bootstrap 会路由到它；"⚠️ noop"表示虽有适配器文件但 bootstrap 仍走 noop。
...
| 事件总线 | `EventBus` | ✅ `goroutineEventBus` | ✅ `goroutineEventBus` | ✅ `goroutineEventBus` |
| 缓存 | `Cache` | ✅ `memoryCache` | ✅ `memoryCache` | ✅ `memoryCache` |
| 可观测性 | `Telemetry` | ✅ `stdoutTelemetry` | ✅ `stdoutTelemetry` | ✅ `stdoutTelemetry` (PrometheusExporter 已实现) |
| 服务发现 | `ServiceRegistry` | ✅ `staticServiceRegistry` | ✅ `staticServiceRegistry` | ✅ `staticServiceRegistry` |
| 灾备 | `DisasterRecovery` | ⚠️ `noopDR` | ✅ `PGDisasterRecovery` (cfg 触发) | ✅ `PGDisasterRecovery` |
| 负载均衡 | `LoadBalancer` | ⚠️ `noopLB` | ✅ `NginxLoadBalancer` (cfg 触发) | ✅ `NginxLoadBalancer` |
| 命令执行 | `CommandExecutor` | ✅ `LocalShellExecutor` | ✅ `DockerSandboxExecutor` (fallback LocalShell) | ✅ `DockerSandboxExecutor` |
| 审批通知 | `Notifier` | ✅ `stdoutNotifier` | ✅ `FeishuNotifier` (cfg 触发) | ✅ `FeishuNotifier` + MultiChannel |
| 依赖缓存 | `DependencyCache` | 无 | ✅ `DependencyCache` (standard+ 启用) | ✅ `DependencyCache` |
...
### 12.1 遥测栈 `[v6 审计: 框架完成, minimal 用 stdout]`

> **[v6 审计]** `internal/observability/` 含 14 文件，框架级实现到位: `domain/circuit_breaker.go` (4.78KB, 3 态熔断器 Closed/Open/HalfOpen) / `domain/load_shedder.go` (3.39KB) / `domain/metrics.go` (888B) / `domain/slo_tracker.go` (975B) / `domain/sharding.go` (3.58KB, HashRing 一致性哈希)。`adapter/prometheus_exporter.go` (3.99KB) 实现 Prometheus 格式导出，已在 `routes.go` 注册 `/metrics` 端点。`adapter/stdout_telemetry.go` (1KB) 为 minimal profile 默认实现。**当前状态**: minimal profile 使用 `stdoutTelemetry`（Trace 返回 noopSpan，Metric 空操作），结构化日志使用 Go 标准库 `slog`（非 zap/zerolog）。OTel/Prometheus 完整集成待 Phase 8。
...
### 19.3 迁移脚本 `[v6 审计: 实际 10 迁移文件]`

> **[v6 审计]** 实际 `migrations/` 目录含 18 文件（9 对 up/down），编号 001-010（无 002）。实际迁移: `001_init` (核心表) / `003_gate_request` (Gate 审批) / `004_learning_tables` (学习系统) / `005_register` (注册扩展) / `006_feature_flags` (特性开关) / `007_soft_delete_index` (软删除索引) / `008_promode` (ProMode 扩展) / `009_user_role_and_invitation` (用户角色+邀请) / `010_gate_stage_width` (Gate stage 宽度)。覆盖 15+ 核心表。设计文档原计划的 002_token_partition / 003_soft_delete / 004_ab_experiment 编号与实际不符。

```
migrations/
  ├── 001_init.up.sql          # Phase 1 表 (project/user/user_role/.../task_queue)
  ├── 001_init.down.sql        # DROP ALL (仅 dev)
  ├── 003_gate_request.up.sql  # Gate 审批请求表
  ├── 004_learning_tables.up.sql # 学习系统表 (trajectory/retrospective/preference/knowledge_snapshot)
  ├── 005_register.up.sql      # 注册扩展
  ├── 006_feature_flags.up.sql # 特性开关表
  ├── 007_soft_delete_index.up.sql # 软删除索引优化
  ├── 008_promode.up.sql       # ProMode 工作台扩展
  ├── 009_user_role_and_invitation.up.sql # 用户角色+邀请系统
  ├── 010_gate_stage_width.up.sql # Gate stage 宽度字段
  └── ... (原 002 编号缺失)
...
#### 18.1.2 能力域实现状态 `[v6 审计: 与 bootstrap.go 对齐]`

> **[v6 审计]** 下表与 `bootstrap.go` 实际路由对照。标注 ✅ 表示该能力域在 minimal profile 下有真实实现；⚠️ 表示有适配器代码但 bootstrap 未路由；❌ 表示仅 noop。

| 能力域 | Phase 1 状态 | 实现 | v6 实际路由 |
|--------|-------------|------|------------|
| `SecretStore` | **真实** | `envfile_store.go` — 读 `.env` 文件, Router 依赖 | ✅ minimal→envfile |
| `CommandExecutor` | **真实** | `local_shell_executor.go` — 宿主机 spawn, BashTool 依赖 | ✅ minimal→local-shell |
| `Telemetry` | **真实** | `stdoutTelemetry` — slog 结构化日志 | ✅ minimal→stdout |
| `Notifier` | **真实** | `stdoutNotifier` — stdout 输出 | ✅ minimal→stdout |
...
| `LoadBalancer` | stub | `noopLB` — 空操作 | ❌ (⚠️ nginx_load_balancer 适配器存在) |
...
> 审计日期: 2026-06-05 | 审计方式: 代码直接扫描（不依赖 DESIGN.md）
> 代码库规模: 208 Go 文件 + 87 前端文件 + 28 Node.js 文件 + 10 迁移 + 70 文档
...
| 15 | `redis_task_queue.go` | TODO(phase-8) | 内存 channel 替代 Redis | Phase 8 |

**低优先级（minimal profile 可接受）:**

| # | 位置 | Stub | 说明 |
|---|------|------|------|
| 16 | `bootstrap.go` | `memoryCache` | 内存 map, minimal 可接受 |
| 17 | `bootstrap.go` | `goroutineEventBus` | goroutine+channel, minimal 可接受 |
| 18 | `bootstrap.go` | `staticServiceRegistry` | 静态发现, minimal 可接受 |
| 19 | `bootstrap.go` | `stdoutNotifier` | stdout 打印, minimal 可接受 |
| 20 | `bootstrap.go` | `envfileSecretStore` | .env 读取, minimal 可接受 |
| 21 | `loader.go` | Ed25519 签名跳过 | `_ = sig; _ = pubKey` |

### A.3 已完成的真实实现

- **认证授权**: JWT 签发/验证/刷新, bcrypt, RBAC 5 角色, 邀请系统 6 端点, 中间件链
- **Pipeline 引擎**: CRUD + 状态机, Gate 审批 + 超时, Stage + Fork, File Lock, Canary, Deploy
- **Agent AI**: QueryEngine 25KB 4 态状态机, PromptBuilder 三层, SkillLoader 16.89KB, 11 工具链, LearningService 学习循环, AB Experiment, ContextCompressor, ErrorRecovery
- **WebSocket**: 首帧认证, 30s 心跳, 11 事件类型, 每 pipeline 独立 QueryEngine
- **LLM 多提供商**: Anthropic (tool use) + DeepSeek (继承改 baseURL) + OpenAI (实现未路由) + Go LLM Router
- **可观测性**: CircuitBreaker 3 态, LoadShedder, SLO Tracker, Prometheus Exporter, HashRing
- **数据库**: 10 迁移 15+ 表, 连接池, MessageBuffer 批量写入, Feature Flags, WORM 审计, 软删除

### A.4 Profile 降级矩阵

| 能力域 | minimal | standard | enterprise |
|--------|---------|----------|------------|
| SecretStore | envfile | envfile | vault-sidecar/ha |
| ContainerRuntime | noop | docker | docker |
| ObjectStore | noop | noop | minio |
| TaskQueue | noop | redis-streams | redis-cluster |
| EventBus | goroutine-chan | goroutine-chan | goroutine-chan |
| Cache | memory | memory | memory |
| Telemetry | stdout | stdout | OpenTelemetry |
| ServiceRegistry | static | static | Consul/etcd |
| DisasterRecovery | noop | pg-streaming | multi-region |
| LoadBalancer | noop | nginx | k8s-ingress |
| Notifier | stdout | feishu | multi-channel |
| CommandExecutor | local-shell | docker-sandbox | docker-sandbox |

### A.5 下一步优先级

1. **安全门禁** (Phase 2/3): OIDC JWT 签名验证, Ed25519 Profile 签名, JWT Secret 环境变量化
2. **LLM 路由修复**: Node.js chat/chatStream 支持多 Provider, TokenMeter 接入查询
3. **面板系统** (Phase 9): 至少实现 chat + diff + terminal 三个核心面板
4. **ProjectPrefs** (Phase 6): `ProjectPrefsLoader` 接入 `of-prefs.yaml` 热重载

---

*附录 A 基于 2026-06-05 代码直接扫描，所有结论可通过对应文件路径验证。*
